...
| Where | Keys |
|---|---|
| Anywhere | `ctrl-r` repos panel · `{`/`}` previous/next repo without leaving the panel · `f` fetch · `p` pull · `P` push (asks before force-with-lease) · `R` rescan · `@` toggle command log |
| Repos | `enter` open the repo's files · `F` fetch all · `U` fast-forward every clean repo that's behind · `o` open in lazygit |
